      ...
      case $value['identity_type'] {
        /^(c|C)ustom (u|U)ser$/: {
          ...
          exec { "Set User-Login \'${value['user']}\' for application \'${key}\'":
            ...
            command  => "&${facts['temp_path']}\\DComPermEx.exe -runas \"${value['appID']}\" \"${value['user']}\" \"${value['password']}\"",
        ...
        /^(i|I)nteractive (u|U)ser$/: {
          exec { "Set User-Login \'Interactive User\' for application \'${key}\'":
            path     => 'C:\Windows\System32\WindowsPowerShell\v1.0',
            command  => "&${facts['temp_path']}\\DComPermEx.exe -runas \"${value['appID']}\" \"Interactive User\"",
        ...
        /^(l|L)aunching (u|U)ser$/: {
          exec { "Set User-Login \'Launching User\' for application \'${key}\'":
            path     => 'C:\Windows\System32\WindowsPowerShell\v1.0',
            command  => "&${facts['temp_path']}\\DComPermEx.exe -runas \"${value['appID']}\" \"Launching User\"",
